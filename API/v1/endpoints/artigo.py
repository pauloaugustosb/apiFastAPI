...
from Core.deps import get_session, get_current_user #   um usuário não pode postar artigo se ele não existir
...
#   GET - Artigo
@router.get('/{artgo_id}', response_model=ArtigoSchema, status_code=status.HTTP_200_OK)
...
async def put_artigo(artigo_id: int, artigo: ArtigoSchema, 
                     db: AsyncSession = Depends(get_session),
    ...
    async with db as session:
        ...
        if artigo_up:
            ...
            #  Caso a atualização seja feita por outro usuário... será o novo mandante do artigo 
